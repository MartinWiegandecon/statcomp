...
1. Pull data on population by group of country of birth and educational attainment (tablename: `cens_11_cobe_r2`). Build a NUTS2-level dataset with information on the total stock of natives (`c_birth = NAT`) and migrants (`c_birth = FOR`) across education groups, and by education groups. In terms of education groups, aggregate the ISCED categories used from Eurostat to four broader categories (no education, primary, secondary and tertiary education). 
